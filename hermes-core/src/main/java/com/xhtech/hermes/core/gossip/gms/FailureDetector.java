...
import com.xhtech.hermes.core.gossip.io.util.FileUtils;
import com.xhtech.hermes.core.gossip.net.ServerAddress;
import com.xhtech.hermes.core.gossip.utils.BoundedStatsDeque;
...
public class FailureDetector implements IFailureDetector, FailureDetectorMBean {
    ...
    private Map<ServerAddress, ArrivalWindow> arrivalSamples_ = new Hashtable<ServerAddress, ArrivalWindow>();
    ...
    public boolean isAlive(ServerAddress ep) {
    ...
    public void report(ServerAddress ep) {
    ...
    public void interpret(ServerAddress ep) {
        ...
        if (phi > phiConvictThreshold_) {
            logger_.trace("notifying listener that {} is down", ep);
    ...
    public void forceConviction(ServerAddress ep) {
    ...
    public void clear(ServerAddress ep) {
        ArrivalWindow heartbeatWindow = arrivalSamples_.get(ep);
        if (heartbeatWindow != null)
            heartbeatWindow.clear();
    }

    public void remove(ServerAddress ep) {
 ...
 */
    ...
    public String getAllEndpointStates() {
        ...
        for (Map.Entry<ServerAddress, EndpointState> entry : Gossiper.instance.endpointStateMap.entrySet()) {
    ...
    public String getEndpointState(String address) throws UnknownHostException {
        StringBuilder sb = new StringBuilder();
        EndpointState endpointState = Gossiper.instance.getEndpointStateForEndpoint(InetSocketAddressUtil.parseInetSocketAddress(address));
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder();
        Set<ServerAddress> eps = arrivalSamples_.keySet();

        sb.append("-----------------------------------------------------------------------");
        for (ServerAddress ep : eps) {
...
class ArrivalWindow {
    ...
    private final double MAX_INTERVAL_IN_MS = GossiperDescriptor.getRpcTimeout();
