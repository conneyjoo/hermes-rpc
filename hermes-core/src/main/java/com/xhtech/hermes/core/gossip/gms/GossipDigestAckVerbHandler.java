...
public class GossipDigestAckVerbHandler implements IVerbHandler {
    ...
    public void doVerb(Message message, String id) {
        ServerAddress from = message.getFrom();
        if (logger_.isTraceEnabled())
            logger_.trace("Received a GossipDigestAckMessage from {}", from);
        if (!Gossiper.instance.isEnabled()) {
            if (logger_.isTraceEnabled())
                logger_.trace("Ignoring GossipDigestAckMessage because consumer is disabled");
            return;
        }
        ...
        try {
            ...
            Map<ServerAddress, EndpointState> epStateMap = gDigestAckMessage.getEndpointStateMap();
            ...
            Map<ServerAddress, EndpointState> deltaEpStateMap = new HashMap<ServerAddress, EndpointState>();
            for (GossipDigest gDigest : gDigestList) {
                ServerAddress addr = gDigest.getEndpoint();
