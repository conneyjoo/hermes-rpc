...
public class IncomingTcpConnection extends Thread {
    ...
    /**
     * A new connection will either stream or mesg for its entire lifetime: because streaming
...
//                Gossiper.instance.setVersion(header.getFrom(), version);   //Receive the first mesg to set the version.
                ...
                // for non-streaming connections, continue to read the mesg (and ignore them) until sender
                // starts sending correct-version mesg (which it can do without reconnecting -- version is per-Message)
//                if (version <= MessagingService.version_) //待理解
                if (true) {
                    Message message = new Message(header, body);
//                  MessagingService.instance().receive(mesg, id);
                    //即：
...
//                    System.out.println("receive mesg,doVerb. Message type:"+verb+",mesg len:"+bodySize+",mesg from:"+mesg.getFrom());
                    verbHandler.doVerb(message, id);
                } else {
                    logger.debug("Received connection from newer protocol version {}. Ignoring mesg", version);
                }
